...
import { StandalonePoolAddressTest } from '../../../types'
import { POOL_BYTECODE_HASH } from './shared/computePoolAddress'
import { expect } from './shared/expect'
import snapshotGasCost from './shared/snapshotGasCost'

describe('PoolAddress', () => {
  let poolAddress: StandalonePoolAddressTest

  const poolAddressTestFixture = async () => {
    const poolAddressTestFactory = await ethers.getContractFactory('StandalonePoolAddressTest')
    return (await poolAddressTestFactory.deploy()) as StandalonePoolAddressTest
  }
