...
async function main() {
    ...
    const factoryProxyAddress = (marginSwapAccountAddresses.accountFactoryProxy as any)[chainId]

    validateAddresses([factoryProxyAddress])

    // get Account Factory proxy
    const diamondFactoryProxy = await ethers.getContractAt(AccountFactoryProxyAbi, factoryProxyAddress, operator)
